...
db.serialize(() => {
  ...
  for (let i = 0; i < 10; i++) {
    stmt.run("harassment", "Je me fais harcellé ha l'aid svp", 15, 1, "http://twitter.com", "On me traite de nom d'oiseux", "MacronDémission", null);
  }
  for (let i = 0; i < 10; i++) {
    stmt.run("fakenews", "Macron démissionne", 14, 3, "http://twitter.com", "Non, Monsieur le président ne démissionne pas.", "ChaudPatate", "http://gouv.fr");
  }
  stmt.finalize();

  db.each("SELECT rowid AS id, type, name, up_count, down_count, link, description, related_user, verified_source FROM post", (err, row) => {
    console.log(row.id + ": " + row.type, row.name, row.up_count, row.down_count, row.link, row.description, row.related_user, row.verified_source);
  });
});
...
app.post("/posts", (req, res) => {
  const { type, name, up_count, down_count, link, description, related_user, verified_source } = req.body;
  const stmt = db.prepare("INSERT INTO post (type, name, up_count, down_count, link, description, related_user, verified_source) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
  stmt.run(type, name, up_count, down_count, link, description, related_user, verified_source, function (err) {
    if (err) {
      res.status(500).json({ error: err.message });
      return;
    }
